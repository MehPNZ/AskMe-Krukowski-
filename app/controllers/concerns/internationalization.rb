...
module Internationalization
  ...
  included do
    ...
    # def current_url(url, locale)
    #   uri = URI(url)
    #   uri_path = uri.path.slice(3,uri.path.length)

    #   if uri.query.present?
    #     curr_url = uri.scheme + "://" + uri.host + ":#{uri.port}/" + locale + uri_path + '?' + uri.query
    #   else
    #     curr_url = uri.scheme + "://" + uri.host + ":#{uri.port}/" + locale + uri_path
    #   end
    # end
